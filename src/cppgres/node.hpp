...
template <node T> struct node_traits {
  static inline bool is(::Node &node) { return false; }
};
...
#define node_dispatch(name)                                                                        \
  if (node_traits<nodes::name>::is(node)) {                                                        \
    std::visit(std::forward<Visitor>(visitor), reinterpret_cast<nodes::name &>(node));             \
  } else

template <typename Visitor> void visit_node(::Node &node, Visitor &&visitor) {
  node_dispatch(List)
node_dispatch(Alias)
